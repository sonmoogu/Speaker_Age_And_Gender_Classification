...
class featureextractor__:
    def measurePitch(voiceID, f0min, f0max, unit):
        ...
        # 음성특성 추출(대표적인 parameter : F0,jitter,shimmer 등등)
        #1. Fo(pitch) : 음성의 높낮이를 일컫는다. 
        # 2. jitter : 성대 진동의 주기가 얼마나 일정한지를 보여줌. 
        # 3. NHR : 음성에 끼여있는 노이즈 
        # 4.shimmer : 성대 진동의 진폭이 얼마나 일정한지를 보여줌
    ...
    def runPCA(df):
        ...
        principalDf = pd.DataFrame(
            data=principalComponents, columns=['shimmerPCA'])
        principalDf
        return principalDf
    ...
    def getfeautre(dir):
        ...
        i = 0
        for wave_file in glob.glob(dir):

            sound = parselmouth.Sound(wave_file)
            (duration, meanF0, stdevF0, hnr, localJitter, localabsoluteJitter, rapJitter, ppq5Jitter, ddpJitter,
             localShimmer, localdbShimmer, apq3Shimmer, aqpq5Shimmer, apq11Shimmer, ddaShimmer) = featureextractor__.measurePitch(
                sound, 75, 300, "Hertz")
            (f1_mean, f2_mean, f3_mean, f4_mean, f1_median, f2_median, f3_median, f4_median) = featureextractor__.measureFormants(
            ...
            selectedfeature = np.hstack((
                                  df['HNR'], df['stdevF0Hz'], df['localShimmer'],df['localJitter']))
            '''selectedfeature_=df[['meanF0Hz'],['HNR'],['localJitter'],['ddaShimmer']].to_numpy()'''

            '''print(features)'''
            i += 1
            new_features=selectedfeature.reshape(-1,1)
            pcaData = featureextractor__.runPCA(df)  # Run jitter and shimmer PCA
            df = pd.concat([df, pcaData], axis=1)  # Add PCA data
            # reload the data so it's all numbers

            df.to_csv("processed_results_____.csv", index=False)
            df = pd.read_csv('processed_results_____.csv', header=0)
            ...
            df['avgFormant'] = (
                df['f1_median'] + df['f2_median'] + df['f3_median'] + df['f4_median']) / 4

            df['mff'] = (df['f1_median'] * df['f2_median'] *
                         df['f3_median'] * df['f4_median']) ** 0.25
            '''
            df.to_csv("processed_results__.csv", index=False)
            df = pd.read_csv('processed_results__.csv', header=0)
            '''
            ...
            '''
            df.to_csv("processed_results2__.csv", index=False)
            '''
            print("finished")
            return new_features
