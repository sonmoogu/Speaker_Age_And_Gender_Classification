    ...
    def getfeautre(dir):
        ...
        for wave_file in glob.glob(dir):
            ...
            (duration, meanF0, stdevF0, hnr, localJitter, localabsoluteJitter, rapJitter, ppq5Jitter, ddpJitter,
             localShimmer, localdbShimmer, apq3Shimmer, aqpq5Shimmer, apq11Shimmer, ddaShimmer) = featureextractor.measurePitch(
                sound, 75, 300, "Hertz")
            (f1_mean, f2_mean, f3_mean, f4_mean, f1_median, f2_median, f3_median, f4_median) = featureextractor.measureFormants(
            ...
            selectedfeature = np.hstack((df['meanF0Hz'],
                                  df['HNR'], df['stdevF0Hz'], df['localShimmer']))
            '''selectedfeature_=df[['meanF0Hz'],['HNR'],['localJitter'],['ddaShimmer']].to_numpy()'''

            '''print(features)'''
            i += 1

            pcaData = featureextractor.runPCA(df)  # Run jitter and shimmer PCA
